...
{
    // Thêm class này để lưu tạm dữ liệu đền bù
    ...
    public class TraPhongService
    {
        ...
        {
            return Db.Query("SELECT d.SoPhieuDat,k.HoTen,d.NgayNhanThucTe,d.NgayTraDuKien FROM PhieuDatPhong d JOIN KhachHang k ON d.MaKhach=k.MaKhach WHERE d.TrangThai=N'Đang ở' ORDER BY d.SoPhieuDat");
        }

        public DataTable LayPhongTheoPhieu(string so)
        {
            return Db.Query("SELECT c.SoPhong,p.DonGiaNgay FROM ChiTietDatPhong c JOIN Phong p ON c.SoPhong=p.SoPhong WHERE c.SoPhieuDat=@s", new SqlParameter("@s", so));
        }

        public DataTable LayTienNghiPhong(string phong)
        {
            return Db.Query(@"SELECT TOP 100 p.MaTienNghi,l.TenLoaiTN,t.TinhTrangHienTai FROM PhieuLapDat p JOIN TienNghi t ON p.MaTienNghi=t.MaTienNghi JOIN LoaiTienNghi l ON t.MaLoaiTN=l.MaLoaiTN WHERE p.SoPhong=@p ORDER BY p.NgayLap DESC", new SqlParameter("@p", phong));
        }
        ...
        // Đã bổ sung  để không bị lỗi CS0305
        ...
        public KetQuaXuLy LapHoaDon(string soHD, string soDat, DateTime ngay, string maNV, int soNgay)
        {
            ...
            try
            {
                decimal phong = Convert.ToDecimal(Db.Scalar("SELECT ISNULL(SUM(p.DonGiaNgay),0) FROM ChiTietDatPhong c JOIN Phong p ON c.SoPhong=p.SoPhong WHERE c.SoPhieuDat=@s", new SqlParameter("@s", soDat))) * soNgay;
                decimal dv = Convert.ToDecimal(Db.Scalar("SELECT ISNULL(SUM(c.ThanhTien),0) FROM PhieuSuDungDV h JOIN ChiTietPhieuSuDungDV c ON h.SoPhieuSDDV=c.SoPhieuSDDV WHERE h.SoPhieuDat=@s", new SqlParameter("@s", soDat)));

                Db.Execute("INSERT INTO HoaDon(SoHoaDon, SoPhieuDat, NgayLap, MaNV, SoNgayTinhTien, TienPhong, TienDichVu, TrangThai) VALUES(@h,@s,@n,@nv,@ng,@p,@d,N'Chưa thanh toán')",
                    new SqlParameter("@h", soHD), new SqlParameter("@s", soDat), new SqlParameter("@n", ngay), new SqlParameter("@nv", maNV), new SqlParameter("@ng", soNgay), new SqlParameter("@p", phong), new SqlParameter("@d", dv));
